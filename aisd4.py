...
# 1После чего если определитель матрицы А больше суммы диагональных элементов матрицы F, то вычисляется выражение: A*AT – K * F-1,
...
if np.linalg.det(A) > np.trace(F):
    print('\nВычисляем выражение A*AT – K * F-1')
    ...
    print('\nВычисляем выражение (A-1 +G-FТ)*K')
    A_trans = A.transpose()
    print('\nТранспонированная матрица A:\n', A_trans)
    G = np.tril(A)
